...
const newRunner = runners.map(runners => runners.first_name.toUpperCase());

console.log(JSON.stringify(newRunner));

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
const largeShirts = runners.filter(run => run.shirt_size === "L");
...
// Problem 1
// Theres not enough space on the paper for all of the information so we would like you to use the forEach method to only return the fist and last names as initials
let initial = [];
runners.map(function(runners){
    initial.push(`${runners.first_name.charAt(0)}.${runners.last_name.charAt(0)}.`);
});
console.log(JSON.stringify(initial));

// Problem 2
// A student has a homework assignment and needs to know how many runners wear a small size shirt or lower. List all the small runners for the student.
const smallShirts = runners.filter(run => run.shirt_size === "S" || run.shirt_size === "XS");
console.log(smallShirts);

// Problem 3
// After a generous donation came in the ticket/donations required dropped 85%. What is the total donation needed to put the race on?
const newTicketPrice = runners.reduce(function (accumulator, currentValue){
    return accumulator + currentValue.donation * .15;
}, 0);
